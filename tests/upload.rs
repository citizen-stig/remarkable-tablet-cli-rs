...
use remarkable_tablet_cli_rs::cli::UploadArgs;
use remarkable_tablet_cli_rs::commands::upload;
use remarkable_tablet_cli_rs::connection::{FakeConnection, TabletConnection};
use remarkable_tablet_cli_rs::error::CliError;
use remarkable_tablet_cli_rs::metadata::{FileType, ItemType, Parent, RawContent, RawMetadata};
...
async fn remote_files(conn: &FakeConnection) -> Vec<String> {
    let mut files = remarkable_tablet_cli_rs::transfer::walk_remote(conn, DATA_DIR)
        .await
        .unwrap();
