...
/// Downcast an `anyhow::Error` to `CliError`. Lower-layer error types
/// (`MetadataError`, `TabletError`) are converted to their `CliError`
/// counterparts via the `From` impls in [`crate::error`]; anything else
/// falls back to `IoError` so unstructured failures still produce a usable
/// JSON envelope.
#[must_use]
pub fn to_cli_error(err: anyhow::Error) -> CliError {
    let err = match err.downcast::<CliError>() {
        Ok(cli) => return cli,
        Err(other) => other,
    };
    let err = match err.downcast::<remarkable_metadata::MetadataError>() {
        Ok(meta) => return meta.into(),
        Err(other) => other,
    };
    match err.downcast::<remarkable_tablet::TabletError>() {
        Ok(tablet) => tablet.into(),
